...
class DataManager:
    def __init__(self, filename, experiment=None, write_mode='w', **kwargs):
        self.experiment = experiment
        self.root = Dataset(filename, write_mode, format='NETCDF4', **kwargs)

    def coord(self, name, value_or_array):
        if name not in self.root.dimension:
            self.root.createDimension(name, len(array))
                if type(value_or_array) is np.ndarray:
                self.root.createVariable(array, name)
