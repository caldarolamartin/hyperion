...
Servo myservo_1;  // create servo object to control a servo; twelve servo objects can be created on most boards
Servo myservo_2;
Servo myservo_3;
Servo myservo_4;
...
const uint8_t pin_flag_2 = 9;

const uint8_t pin_servo_1 = 10;
const uint8_t pin_servo_2 = 1;
const uint8_t pin_servo_3 = 2;
const uint8_t pin_servo_4 = 3;
...
void setup() {
  ...
  myservo_1.attach(pin_servo_1);  // attaches the servo on pin ... to the servo object
  myservo_2.attach(pin_servo_2);
  myservo_3.attach(pin_servo_3);
  myservo_4.attach(pin_servo_4);
//
//  flip_servo(in_path);
//
//  delay(1000);
...
void loop() {
  ...
  if (!digitalRead(pin_toggle_1_green)) {
    ...
    if (millis() - timestamp1 > debounce_timeout_ms) {  // if timout expired
      if (switch1_down != 11) set_flag(1,green);  // if button not already down
      switch1_down = 1;
    }
  ...
  if (button_pin>=0) {                                    // if switch 1 appears to be in rest position
   ...
   if (button_pin<=0) {                                   // if switch 2 appears to be in rest position
...
void set_flag(uint8_t sflag, bool state) {
  if (sflag==1) {
    if (flag_1_state != state) {
      ...
      // Place code or refer to function here that performs the action to go to the new state
      //digitalWrite(pin_flag_1, flag_1_state);
      flip_servo_1(flag_1_state);
      // END of state flipping code
...
void flip_servo_1(bool in_path) {
  if (in_path){
    myservo_1.write(40);
  }
  else {
...
void flip_servo_2(bool in_path) {
  if (in_path){
    myservo_1.write(40);
  }
  else {
